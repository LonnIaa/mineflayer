...
// Indexed by the 3D direction data value (DOWN 0, UP 1, NORTH 2, SOUTH 3, WEST 4, EAST 5): the
// facing carried in the modern (1.21.9+) spawn_entity data field for paintings. DOWN/UP won't
// occur for a wall-hung painting but are included so the lookup never returns undefined.
const paintingFace3DToVec = [
...
function inject (bot, { version, storageBuilder, hideErrors }) {
  ...
  function addColumn (args) {
    ...
    try {
      ...
      // Retain the parsed heightmaps for optional consumers (pathfinding shortcuts,
      // mods). pchunk stores them opaquely (no decode/query) and does not persist
      // them; block/biome/light decode above is entirely independent of this. Guarded
      // because only the pc/1.18+ ChunkColumn exposes the helper.
      if (args.heightmaps !== undefined && typeof column.loadHeightmaps === 'function') {
        column.loadHeightmaps(args.heightmaps)
      }
  ...
  // The vanilla client caps decoded chunk-data at 2 MiB — it checks the length and throws BEFORE
  // allocating the byte array. minecraft-protocol types chunkData as an unbounded ByteArray, so
  // mirror that bound here defensively: a hostile/broken server sending a huge blob would
  // otherwise force a large allocation + a doomed chunk decode. Reject the whole column cleanly
  // (skip addColumn AND its block entities, as vanilla aborts the read).
  const CHUNK_DATA_MAX_BYTES = 2097152
  bot._client.on('map_chunk', (packet) => {
    if (packet.chunkData && packet.chunkData.length > CHUNK_DATA_MAX_BYTES) {
      if (!hideErrors) console.warn(`Ignoring oversized chunk at ${packet.x}, ${packet.z}: chunkData is ${packet.chunkData.length} bytes, exceeds the ${CHUNK_DATA_MAX_BYTES}-byte (2 MiB) cap`)
      return
    }
    addColumn({
      x: packet.x,
  ...
  // chunk_biomes (0x0d, ClientboundChunksBiomesPacket): biome-only rewrite of
  // already-loaded columns (e.g. /fillbiome, worldgen border retrofit). Each
  // entry's `data` blob holds one biome PalettedContainer per section; apply it
  // via the column's replaceBiomes helper (vanilla ClientPacketListener.handleChunksBiomes
  // -> ChunkSource.replaceBiomes -> LevelChunk.replaceBiomes).
  bot._client.on('chunk_biomes', (packet) => {
    for (const entry of packet.biomes) {
      const column = bot.world.getColumn(entry.position.x, entry.position.z)
      if (!column) {
        if (!hideErrors) console.warn('Ignoring chunk_biomes for unloaded chunk at', entry.position.x, entry.position.z)
        continue
      }
      if (typeof column.replaceBiomes !== 'function') return // older pchunk without the helper
      try {
        column.replaceBiomes(entry.data)
      } catch (e) {
        bot.emit('error', e)
        continue
      }
      // Re-store the (mutated-in-place) column so listeners get a chunkColumnLoad refresh.
      bot.world.setColumn(entry.position.x, entry.position.z, column)
    }
  })
  ...
  bot._client.on('multi_block_change', (packet) => {
    ...
    for (let i = 0; i < packet.records.length; ++i) {
      ...
      if (bot.supportFeature('usesMultiblockSingleLong')) {
        // Vanilla wire is a VarLong: stateId = packedChange >>> 12 (ClientboundSectionBlocksUpdatePacket:50).
        // Divide instead of `>> 12` so the stateId stays correct for records >= 2^31 (a 32-bit `>>` sign-corrupts).
        updateBlockState(pt, Math.floor(record / 4096))
      } else {
  ...
  // 1.21.9+ (26.x) removed the dedicated spawn_entity_painting packet; paintings now spawn
  // through the generic spawn_entity packet with entity type 'painting'. Vanilla puts the
  // facing in the spawn packet's data field as the 3D direction data value and the position as
  // f64 x/y/z. The painting variant is not in the spawn packet — it arrives later via the
  // painting-variant entity metadata — so name is unknown at spawn time.
  bot._client.on('spawn_entity', (packet) => {
    if (bot.registry.entities[packet.type]?.name !== 'painting') return
    const pos = new Vec3(packet.x, packet.y, packet.z)
    const dataField = typeof packet.objectData === 'number'
      ? packet.objectData
      : (typeof packet.data === 'number' ? packet.data : 0)
    const painting = new Painting(packet.entityId, pos, packet.title ?? null, paintingFace3DToVec[dataField] ?? null)
    addPainting(painting)
  })
  ...
  bot._client.on('respawn', (packet) => {
    ...
    // A respawn that reaches here is a real dimension/world change (the guards above
    // return early on a same-dimension respawn). Signal the world swap so stale entities
    // from the old world and their physics-prediction contexts are dropped (entities.js
    // and entity_physics.js listen for 'worldSwitch'); vanilla discards the old ClientLevel.
