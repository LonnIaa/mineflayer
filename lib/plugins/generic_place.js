...
function inject (bot) {
  ...
  async function _genericPlace (referenceBlock, faceVector, options) {
    ...
    // In the vanilla client the use_item_on packet is sent DURING the tick and the player tick
    // then sends its position packet LATER THE SAME TICK, so a flying packet always follows the
    // placement before any server transaction. Strict anti-cheat servers queue post-actions
    // (like a block placement) and expect the tick's trailing movement packet to arrive before
    // the next transaction; a mineflayer bot standing still emits no following flying packet
    // that tick (the status-only send is gated on an onGround/collision change), which reads as
    // an out-of-order action. Mirror vanilla by emitting the trailing per-tick flying
    // (status-only) immediately after the placement. Carries the real
    // onGround/horizontalCollision exactly like physics.js's per-tick send.
    if (bot.entity) {
      const og = bot.entity.onGround === true
      bot._client.write('flying', {
        onGround: og,
        flags: { onGround: og, hasHorizontalCollision: bot.entity.isCollidedHorizontally === true } // 1.21.3+
      })
    }
