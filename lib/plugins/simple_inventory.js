...
function inject (bot) {
  ...
  function setQuickBarSlot (slot) {
    ...
    // In the vanilla client the carried-item change is sent DURING the tick and the player tick
    // sends its position packet later the SAME tick, so a flying packet always follows the slot
    // change before any server transaction (same packet-order expectation as the block-place
    // trailing send in generic_place.js). A still mineflayer bot emits no trailing flying that
    // tick, which strict anti-cheat servers read as an out-of-order held-item change. Mirror
    // vanilla: emit the trailing per-tick flying (status-only) immediately after the slot change.
    if (bot.entity) {
