...
function inject (bot, { hideErrors }) {
  ...
  bot._client.on('set_cooldown', (packet) => {
    // 1.21.2+ (26.2): the packet references a cooldown GROUP (Identifier string), not an item id.
    // packet = { cooldownGroup: string, cooldownTicks: varint }. The old `packet.itemID` no longer
    // exists (it was always undefined -> the handler early-returned forever). An item's group is its
    // use_cooldown component's cooldownGroup override, defaulting to the item's registry id.
    if (bot.heldItem) {
      const useCooldown = bot.heldItem.componentMap?.get('use_cooldown')
      const heldGroup = useCooldown?.data?.cooldownGroup ?? `minecraft:${bot.heldItem.name}`
      if (heldGroup !== packet.cooldownGroup) return
    }
    if (!eatingTask.done) {
      eatingTask.finish()
    }
  ...
  function activateItem (offHand = false) {
    ...
    if (bot.supportFeature('useItemWithBlockPlace')) {
      ...
      // On 1.21+ the use_item packet carries the player's yaw/pitch, and strict servers require
      // it to EQUAL the same tick's flying-packet rotation (the vanilla client sends the same
      // rotation field in both). mineflayer's flying packet sends a yaw-speed-clamped +
      // wire-wrapped WIRE rotation (physics.js lastSentYaw -> _lastSentRotation), which diverges
      // from the raw look TARGET bot.entity.yaw. Use the SAME wire rotation the move packet last
      // sent so the two packets agree; fall back to the raw conversion only before the first send.
  ...
  async function clickWindow (slot, mouseButton, mode) {
    ...
    if (bot.supportFeature('transactionPacketExists')) {
      ...
      // 1.21.5+ serverbound container-click uses HashedSlot (per-component CRC32C hash,
      // empty = option-absent) instead of a full Slot. Item.toNotch would write a present
      // itemId-0 slot for empties and a 0 hash for component items -> server slot desync.
      const clickToNotch = bot.registry.version['>=']('1.21.5') ? Item.toNotchHashed : Item.toNotch

      changedSlots = changedSlots.map(slot => {
        return {
          location: slot,
          item: clickToNotch(window.slots[slot])
        }
      })
    }
    ...
    if (bot.supportFeature('stateIdUsed')) { // 1.17.1 +
      const cursorToNotch = bot.registry.version['>=']('1.21.5') ? Item.toNotchHashed : Item.toNotch
      bot._client.write('window_click', {
        ...
        mode,
        changedSlots,
        cursorItem: cursorToNotch(window.selectedItem)
      })
  ...
  bot._client.on('open_horse_window', (packet) => {
    // open window
    // 26.2 renamed the packet fields: windowId -> containerId, and the slot field
    // nbSlots -> inventoryColumns (the wire now carries the number of chest COLUMNS,
    // not a slot count). Derive the horse-window slot count the way vanilla builds the
    // menu (HorseInventoryMenu): 2 fixed slots (saddle + body armor) + 3 rows * columns
    // chest slots. Fall back to the old fields for pre-26.2 protocols.
    const containerId = packet.containerId ?? packet.windowId
    const nbSlots = packet.inventoryColumns !== undefined
      ? 2 + packet.inventoryColumns * 3
      : packet.nbSlots
    bot.currentWindow = windows.createWindow(containerId, 'HorseWindow', 'Horse', nbSlots)
    prepareWindow(bot.currentWindow)
  })
